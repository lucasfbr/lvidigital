...
/**
 * Class SeoPlan_Walker_Mega_Menu
 *
...
class SeoPlan_Walker_Mega_Menu extends Walker_Nav_Menu {
	...
	public function start_lvl( &$output, $depth = 0, $args = array() ) {
		...
		if ( ! $depth && $this->in_mega ) {
			...
			$output .= "\n$indent<ul class=\"sub-menu wpf-mega-menu\" $style>\n";
		} else {
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		$item_mega = seoplan_parse_args( $item_mega, seoplan_get_mega_menu_setting_default() );
		...
		{
			$narrow_down = '<i class="fa fa-angle-down" aria-hidden="true"></i>';
		}
		$item_output = $args->before;
		$item_output .= $link_open;
		$item_output .= $args->link_before . $icon . $title . $narrow_down . $args->link_after;
 ...
 * @return array
 */
function seoplan_nav_menu_args( $args ) {
	if ( ! in_array( $args['theme_location'], array( 'topbar', 'footer', 'socials' ) ) ) {
		$args['walker'] = new SeoPlan_Walker_Mega_Menu;
	}
...
add_filter( 'wp_nav_menu_args', 'seoplan_nav_menu_args' );