	...
	smm = {
		...
		initTemplates: function () {
			_.each( smmModals, function ( name ) {
				smm.templates[name] = wp.template( 'seoplan-' + name );
			} );
		},
		...
		saveChanges: function () {
			var data = smm.$modal.find( '.smm-content :input' ).serialize(),
				$spinner = smm.$modal.find( '.smm-toolbar .spinner' );

			$spinner.addClass( 'is-active' );
			$.post( ajaxurl, {
				action: 'seoplan_save_menu_item_data',
				data  : data
			}, function ( res ) {
				if ( !res.success ) {
					return;
